...
class LLMDeploymentService:
    ...
    def create_vllm_environment(self, deployment: LLMDeployment) -> bool:
        ...
        try:
            ...
            self.log(deployment, "Step 1/7: Upgrading pip")
            subprocess.run(
                [str(pip_path), "install", "--upgrade", "pip"],
                check=True,
                capture_output=True,
            ...
            self.log(deployment, "Step 2/7: Installing CPU PyTorch (this may take 10-20 minutes)")
            subprocess.run(
            ...
            self.log(deployment, "Step 3/7: Installing Hugging Face utilities")
            subprocess.run(
            ...
            if not repo_dir.exists():
                self.log(deployment, "Step 4/7: Cloning vLLM source code")
                subprocess.run(
            ...
            if cpu_build_req.exists():
                self.log(deployment, "Step 5/7: Installing CPU build requirements")
                subprocess.run(
                ...
                self.log(deployment, "Step 5/7: Installing fallback build requirements")
                subprocess.run(
            ...
            if cpu_req.exists():
                self.log(deployment, "Step 6/7: Installing CPU runtime requirements")
                subprocess.run(
                ...
                self.log(deployment, "Step 6/7: Installing fallback runtime requirements")
                subprocess.run(
            ...
            self.log(deployment, "Step 7/7: Building vLLM from source (this may take 30-60 minutes)")
            subprocess.run(
