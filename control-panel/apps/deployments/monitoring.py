...
Reference: CLAUDE.md "Monitoring & Logs" section
"""
...
from apps.core.models import SystemHealthCheck, SecurityAuditLog
...
class DeploymentMonitor:
    ...
    def check_deployment_health(self, deployment: Deployment) -> Dict[str, Any]:
        """
        ...
        if deployment.status != Deployment.Status.RUNNING:
            if deployment.status == Deployment.Status.FAILED:
                ...
                health_status['issues'].append('Deployment failed')
            elif deployment.status == Deployment.Status.BUILDING:
                # Check build time
    ...
    def get_deployment_metrics(self, deployment: Deployment) -> Dict[str, Any]:
        """
        ...
        for deployment in deployments:
            try:
                ...
                if not health_status['is_healthy']:
                    self.send_alert(
                        subject=f"Deployment Issue: {deployment.name}",
...
class DeploymentAnalytics:
    ...
    def get_deployment_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        deployments = Deployment.objects.filter(created_at__gte=since)
        
        stats = {
            'period_days': days,
            'total_deployments': deployments.count(),
            'successful_deployments': deployments.filter(status=Deployment.Status.RUNNING).count(),
            'failed_deployments': deployments.filter(status=Deployment.Status.FAILED).count(),
            'pending_deployments': deployments.filter(status=Deployment.Status.PENDING).count(),
            'building_deployments': deployments.filter(status=Deployment.Status.BUILDING).count(),
            'by_project_type': {},
            'by_user': {},
            'success_rate': 0.0,
            'avg_build_time': 0.0,
        ...
        for project_type in Deployment.ProjectType:
