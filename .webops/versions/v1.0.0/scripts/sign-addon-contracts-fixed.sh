...
sign_addon_contract() {
    ...
    # Source the addon in a subshell to avoid local variable issues
    (
        # Temporarily replace 'local' with regular variable declarations
        # This is a workaround for the sourcing issue
        alias local='declare' 2>/dev/null || true
...
main() {
    ...
    log_info "WebOps Addon Contract Signing Script (Fixed)"
    log_info "=============================================="
    ...
    if [[ -n "$target_addon" ]]; then
        ...
        for addon in "${addons[@]}"; do
            log_info "Processing addon: $addon"
            if sign_addon_contract "$addon"; then
                ((signed_count++))
                log_info "Successfully signed: $addon"
            else
                ((failed_count++))
                log_error "Failed to sign: $addon"
            fi
            echo
        done
